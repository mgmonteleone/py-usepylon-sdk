...
- `sub-agent-bug-fixer` → invokes `bug-fixer` agent (also handles test gaps)
...
### Phase 2: Documentation & Test Coverage (Parallel)

Run these tasks in parallel after review comments are resolved:
...
2. **Test Coverage Check**:
   - Run `pytest --cov` to check coverage on changed files
   - If coverage gaps exist, dispatch `sub-agent-bug-fixer` with instructions to add missing tests
...
2. **Pre-Merge Verification**:
   ...
   - **Verify `uv lock --check` passes** - lock file must be in sync with pyproject.toml
