...
description: "Testing standards, pytest patterns, and coverage requirements"
---
...
## HTTP Mocking for SDK Tests

Use `respx` for mocking HTTP requests in SDK tests:
...
def test_get_issue_returns_issue():
    """Test SDK correctly parses API response."""
    respx.get("https://api.usepylon.com/issues/123").mock(
        return_value=Response(200, json={"id": "123", "title": "Test"})
    )

    client = PylonClient(api_key="test")
...
- Order tests so fast unit tests run first
- Use `pytest.mark.slow` for integration tests
- Ensure mocks faithfully represent real API behavior