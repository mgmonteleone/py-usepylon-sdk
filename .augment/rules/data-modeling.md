...
- Create separate Pydantic models for API responses vs internal domain models
- Use `model_validate()` for data conversion (replaces deprecated `from_orm()` from Pydantic v1)
- Keep API response models and internal models decoupled

### API Response to Pydantic Conversion
...
# Convert API response dict to Pydantic model
def parse_user_response(data: dict) -> UserResponse:
